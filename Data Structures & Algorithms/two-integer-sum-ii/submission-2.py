    ...
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        while front < end: #ensures that we won't pass the other pointer if the situation goes that long
            if (numbers[front] + numbers[end] == target) is True: 
                return [front+1, end+1] # returns the indexes if the target is meet, note the (+1) is to make it make sense to an non programmer that doesn't know it starts counting from zero.
                ...
                end -= 1 # Progress the end pointer as the number needs to get smaller rather than bigger.
